...
;(async () => {
  const [
    ...
    uint8ArrayToString,
//    stdinToStream './stream.mjs'
//    streamToConsole './stream.mjs'
  ] = await Promise.all([
    (await import('libp2p')).createLibp2p,
    (await import('@libp2p/tcp')).tcp,
    (await import('@libp2p/mplex')).mplex,
    (await import('@chainsafe/libp2p-yamux')).yamux,
    (await import('@chainsafe/libp2p-noise')).noise,
    (await import('@libp2p/floodsub')).floodsub,
    (await import('@libp2p/bootstrap')).bootstrap,
    (await import('@libp2p/pubsub-peer-discovery')).pubsubPeerDiscovery,
    (await import('libp2p/circuit-relay')).circuitRelayTransport,
    (await import('libp2p/circuit-relay')).circuitRelayServer,
...
//import { stdinToStream, streamToConsole } from './stream.mjs'
  ...
  console.log(DHT);
  console.log(uint8ArrayFromString);
